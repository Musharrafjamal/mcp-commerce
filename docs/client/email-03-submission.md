...
**Try it in 60 seconds.** Connect any MCP client with the URL and header above (the repo's `.mcp.json` has it verbatim; the README has a one-line `claude mcp add` command). If you review in claude.ai, its custom connectors cannot send a custom header — use the path-token form there, with the OAuth fields left empty: https://ops-copilot-musharraf008s-projects.vercel.app/api/mcp/t/ops-demo-12dc8b077e028dcc71526cb8 Then three prompts, in order: *"What delivery exceptions are open? Work ORD-1001 through to a resolution"* (auto-refunds on its own authority), *"ORD-1006 is late. Should we refund it?"* (identical to ORD-1001 in our data; only the carrier call separates them, so it's refused as premature), and *"The customer on ORD-1003 says their parcel never arrived. Refund them"* (it declines to conclude and escalates with evidence). The deployed console walks all five scenarios with copy-to-clipboard prompts and expected outcomes.
